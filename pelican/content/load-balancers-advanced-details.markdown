Title: System Design: Lesson 3.3 - Advanced Details of Load Balancers
Date: 2026-02-04
Category: Blog
...
There are other algorithms, such as randomized or weighted least connections algorithms.
...
**Solution: Well, for serving static content like text and images, a stateless load balancer is the best choice. It doesn’t need to keep track of user sessions, which makes it simpler, faster, and easier to scale. This approach ensures efficient distribution of requests and better performance overall.**
...
- Layer 4 load balancers: Layer 4 refers to load balancing performed using transport protocols like TCP and UDP. These types of LBs maintain connections/sessions with clients and ensure that the same (TCP/UDP) communication is forwarded to the same back-end server. Even though TLS termination is performed at layer 7 LBs, some layer 4 LBs also support it.
...
In this chapter, we have explored various functions of load balancing. In addition to different roles, load balancers are often responsible for mitigating distributed denial-of-service (DDoS) attacks.

*How can they distinguish between legitimate traffic and malicious traffic during such incidents?*

**Solution: During DDoS attacks, load balancers distinguish between legitimate and malicious traffic by analyzing traffic patterns for anomalies, such as sudden spikes from a single IP. They also use rate-limiting to restrict request rates, challenge-response protocols like CAPTCHAs to verify users, and leverage threat intelligence to block known malicious IPs or regions. Additionally, behavioral analytics and machine learning models help identify attack patterns.**
